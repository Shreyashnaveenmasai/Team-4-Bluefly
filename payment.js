...
var billingAddressForm = document.querySelector("#adres");
...
var creditCardOption = document.querySelector(
  '#payments .option[data-value="credit-card"]'
);
var paypalOption = document.querySelector(
  '#payments .option[data-value="paypal"]'
);
...
function apply(data, total) {
  ...
  if (
    ...
    if (localStorage.getItem("fee") === "FREE") {
      document.querySelector("#pcee").textContent =
        "$" + Math.round(total + 70);
    } else {
      document.querySelector("#pcee").textContent =
        "$" + Math.round(total + 70 + 20);
    }
    document.querySelector("#h44").textContent = "Promo Applied";
